...
鼠标移到面板上沿（顶部灰色短横线附近，光标变为移动图标）时可按住拖动面板，面板始终保持在 Typora 窗口内，位置会被记住。右上角“−”将面板缩小为一行小矩形，保留标题、“□”展开按钮和“×”关闭按钮；点击“□”或按 `Ctrl+Alt+R` 恢复完整面板。点击“×”会先提示快捷键，确认后隐藏面板与右下角入口；按 `Ctrl+Alt+R` 可重新显示。关闭和缩小都不会停止录音或服务。实时识别预览位于“本地识别 · 在线润色后自动补充”正下方。
...
3. 备份原安装目录，将新版文件覆盖到同一目录，保留 `config.local.json`、`.asr` 及已有的录音和逐字稿（旧版保存在文档同目录，新版默认在 `~/.TyporaASR/RecordData`）。
...
- 点击“查看逐字稿”查看/复制只读原文；资料记录目录中的 `<文档名>.逐字稿-yyyy-MM-dd_HH-mm-ss-fff.md` 自动保存带时间戳的原始逐字稿。该文件由服务管理，请在需要手工编辑时先复制。人工编辑目标 Markdown 不会修改原文。
...
- 资料记录目录：默认是用户主目录下的 `.TyporaASR/RecordData`，可在面板“配置管理 → 资料记录目录”中点击“选择目录”修改，选定后立即保存；“恢复默认”可改回默认目录。修改只对之后开始的录音生效，已有录音和逐字稿保留在原位置；旧版保存在文档旁、尚未建立映射的会话，恢复时会移到资料记录目录。
- 配置导出/导入：面板“配置管理”中的“配置导出”把在线润色设置、在线 ASR 模型设置和资料记录目录保存为 JSON 文件，**文件包含明文 API Key，请妥善保管**；“配置导入”从该文件恢复。导入前会先校验全部设置，有错误时不做任何修改；文件中 Key 为空时保留本机已保存的 Key；在线 ASR 只有连接测试通过才会启用；目录在本机无法创建时保留原目录并提示。
- `<文档名>.录音-yyyy-MM-dd_HH-mm-ss-fff.wav`：保存在资料记录目录，名称使用录音开始时间（精确到毫秒），标准 WAV，16kHz、16bit、单声道。暂停续录及恢复保持同一名称；同毫秒冲突时加数字后缀。旧会话恢复时自动改名，优先使用已记录的开始时间，历史无时钟信息时使用旧录音文件的创建时间。
- `<文档名>.逐字稿-yyyy-MM-dd_HH-mm-ss-fff.md`：保存在资料记录目录的原始逐字稿，名称时间戳及重名后缀与对应 WAV 一致；暂停续录保持同一文件，旧会话恢复时自动改名。
- 旧 `.asr/audio/*.pcm`：恢复会话时转换为资料记录目录中的 WAV，旧文件保留作备份。
