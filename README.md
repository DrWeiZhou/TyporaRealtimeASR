...
通信使用 200ms 轮询获取有序事件与状态，具有游标重放、窗口租约和认证；未引入 WebSocket。模型推理全局串行，预览可以合并，final 持久排队。长句在约 15 秒到达强制边界时，两侧文本保留为待确认，不自动尝试不可靠的文字去重。
...
句尾提交：在约 450ms 的明显停顿后确认片段，不再依据预览标点和 160ms 短停顿提前截取音频，避免将犹豫和静音误当成新句。正式片段会取消当前预览，优先识别；润色完成后立即进入插入与保存流程，不等结束录音。实际延迟还包括本地推理、在线 LLM 响应及安全编辑状态等待。

在线润色单段连续失败 3 次后保留原文和失败状态，后续片段继续处理与入文；点击“重试润色”可补处理失败段，已写入内容不会重复插入。补写的失败段追加到文末，不重排已人工编辑的正文。
