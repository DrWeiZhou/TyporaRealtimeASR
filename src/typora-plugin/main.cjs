...
module.exports=function mount(w,config){
 ...
 let adapter,controller,session,busy=false,controlBusy=false,controlRevision=0,replayRevision=0,after=0,lastSave=0,lastStatus='',pending=new Map(),toSave=new Map(),observed=new Set(),retrying=new Set(),lastPolishCompleted;
 ...
 const panel=d.createElement('section');panel.id='asr-panel';panel.innerHTML=`<h3>语音记录</h3><small>本地识别 · 在线润色后自动补充</small><p id="asr-target">尚未绑定文档</p><select id="asr-devices" aria-label="麦克风"><option value="-1">默认麦克风</option></select><div><button id="asr-start">开始录音</button><button id="asr-pause" disabled>暂停</button><button id="asr-stop" disabled>结束录音</button><button id="asr-recover">恢复记录</button></div><p id="asr-status">准备就绪。Ctrl+Alt+R 显示或收起面板。</p><div id="asr-preview">当前句将在这里实时显示。</div><div id="asr-review"></div>`;
 ...
 async function recover(id){
   if(busy||controlBusy)return;busy=true;try{
 ...
 function review(event){
   ...
   const insert=d.createElement('button');insert.textContent='插入此段';insert.onclick=async()=>{if(busy||controlBusy)return;busy=true;try{const result=await controller.apply(event,true);if(result!=='deferred'){pending.delete(event.eventId);toSave.set(event.eventId,event);card.remove()}}catch(e){showError(e)}finally{busy=false}};
   const ignore=d.createElement('button');ignore.textContent='忽略';ignore.onclick=async()=>{if(busy||controlBusy)return;busy=true;try{await ack(event.eventId,'deleted');pending.delete(event.eventId);card.remove()}catch(e){showError(e)}finally{busy=false}};
   card.append(note,text,insert,ignore);area.append(card);
 }
 async function tick(){
   if(busy||controlBusy||!session)return;busy=true;
   try {
    ...
    if(pending.size<200){const revision=replayRevision;const events=await client.request('GET',`/sessions/${session.sessionId}/events?after=${after}`);if(revision!==replayRevision)return;for(const e of events){if(e.polishState==='failed')retrying.add(e.eventId);pending.set(e.eventId,e);after=Math.max(after,e.seq)}}
    ...
    for(const [id,event] of pending){
      ...
      if(result==='failed'){retrying.add(id);pending.delete(id);continue;}
      retrying.delete(id);
