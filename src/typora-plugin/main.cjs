...
module.exports=function mount(w,config){
 ...
 let adapter,controller,session,busy=false,after=0,lastSave=0,lastStatus='',pending=new Map(),toSave=new Map(),observed=new Set();
 ...
 const panel=d.createElement('section');panel.id='asr-panel';panel.innerHTML=`<h3>语音记录</h3><small>本地识别 · 已确认的短句自动补充</small><p id="asr-target">尚未绑定文档</p><select id="asr-devices" aria-label="麦克风"><option value="0">默认麦克风</option></select><div><button id="asr-start">开始录音</button><button id="asr-stop" disabled>停止录音</button><button id="asr-recover">恢复记录</button></div><p id="asr-status">准备就绪。Ctrl+Alt+R 显示或收起面板。</p><div id="asr-preview">当前句将在这里实时显示。</div><div id="asr-review"></div>`;
 ...
 async function start(){
  if(busy)return;busy=true;$('asr-start').disabled=true;
  try {
   if(pending.size||toSave.size)throw new Error('请先处理上一会话的待写入或未保存内容');
   ...
   after=0;pending.clear();toSave.clear();observed.clear();remember(documentId);$('asr-stop').disabled=false;status('正在录音，你可以同时编辑已有内容。');
  }catch(e){showError(e);$('asr-start').disabled=false}finally{busy=false}
 }
 async function stop(){if(!session)return;try{status('正在保存尾音…');await client.request('POST',`/sessions/${session.sessionId}/stop`,{});$('asr-stop').disabled=true;status('录音已停止，正在处理剩余音频。')}catch(e){showError(e)}}
 async function recover(id){
   if(busy)return;busy=true;try{
    if(session?.recording)throw new Error('请先停止当前录音');
 ...
 function review(event){
   ...
   const note=d.createElement('small');note.textContent=event.needsReview?'长句边界需要确认':'此段可能已插入或被人工删除，请核对后选择';
   const insert=d.createElement('button');insert.textContent='插入此段';insert.onclick=async()=>{try{if(busy)return;busy=true;const result=await controller.apply(event,true);if(result!=='deferred'){pending.delete(event.eventId);toSave.set(event.eventId,event);card.remove()}}catch(e){showError(e)}finally{busy=false}};
   const ignore=d.createElement('button');ignore.textContent='忽略';ignore.onclick=async()=>{try{await ack(event.eventId,'deleted');pending.delete(event.eventId);card.remove()}catch(e){showError(e)}};
   card.append(note,text,insert,ignore);area.append(card);
 }
 async function tick(){
   if(busy||!session)return;busy=true;
   try {
    session=await client.request('GET',`/sessions/${session.sessionId}`);
    $('asr-stop').disabled=!session.recording;$('asr-start').disabled=session.recording||session.pending>0;
    if(session.hypothesis)$('asr-preview').textContent=session.hypothesis.text;
    else if(!session.recording)$('asr-preview').textContent='录音已停止。';
    ...
    for(const [id,event] of pending){
      ...
      if(result==='review'){review(event);continue;}
    ...
    status(session.error||`${session.recording?'录音中':'录音已停止'} · ${Math.floor(session.seconds)} 秒 · 待识别 ${session.pending} 段 · 待确认 ${pending.size} 段 · 未保存 ${toSave.size} 段`);
   }catch(e){showError(e)}finally{busy=false}
 }
 $('asr-start').onclick=start;$('asr-stop').onclick=stop;$('asr-recover').onclick=()=>recover();
 refreshDevices();const timer=w.setInterval(tick,500);
 const api={start,stop,recover,getState:()=>({session,pending:pending.size,toSave:toSave.size,status:lastStatus}),dispose:()=>{w.clearInterval(timer);adapter?.dispose();panel.remove();toggle.remove();style.remove()}};
