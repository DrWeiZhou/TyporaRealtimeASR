...
module.exports=function controls(w,config,client,panel,hooks){
 ...
 $('asr-raw-open').onclick=async()=>{const s=hooks.session();if(!s)return;try{const raw=await client.request('GET',`/sessions/${s.sessionId}/transcript`);$('asr-raw-text').textContent=raw.text||'';$('asr-raw-view').open=true;if(!raw.path||!fs.existsSync(raw.path)){hooks.status('逐字稿文件尚不存在');return;}try{const electron=w.reqnode('electron');const err=await electron.shell.openPath(raw.path);if(err)throw new Error(err);}catch{spawn('cmd',['/c','start','',raw.path],{windowsHide:true,shell:false,stdio:'ignore'});}hooks.status('已打开逐字稿');}catch(e){hooks.status(e.message||String(e));}};
