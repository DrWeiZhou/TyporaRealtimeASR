...
module.exports=function controls(w,config,client,panel,hooks){
 ...
 $('asr-service-start').onclick=async()=>{
  ...
  try{await launcher.start();await hooks.refreshDevices();await loadConfig();await loadAsrConfig().catch(()=>{});}
 ...
 $('asr-raw').onclick=async()=>{const s=hooks.session();if(!s)return;try{const raw=await client.request('GET',`/sessions/${s.sessionId}/transcript`);$('asr-raw-text').textContent=raw.text;$('asr-raw-view').open=true;}catch(e){hooks.status(e.message);}}; function resolveTranscriptFile(){ const doc=w.File&&w.File.bundle&&w.File.bundle.filePath; if(!doc)throw new Error('\u8bf7\u5148\u4fdd\u5b58\u5e76\u6253\u5f00 Markdown \u6587\u6863'); const dir=path.dirname(doc); const base=path.basename(doc,path.extname(doc)); const marker='\u9010\u5b57\u7a3f'; const prefix=base+'.'+marker+'-'; const names=fs.readdirSync(dir).filter(n=>n.startsWith(prefix)&&n.endsWith('.md')); if(!names.length)throw new Error('\u5f53\u524d\u6587\u6863\u65c1\u6ca1\u6709\u9010\u5b57\u7a3f\u6587\u4ef6'); names.sort(); return path.join(dir,names[names.length-1]); } async function openTranscriptFile(){ try{ hooks.status('\u6b63\u5728\u6253\u5f00\u9010\u5b57\u7a3f\u2026'); let file=null; const s=hooks.session(); if(s&&s.sessionId){ try{ const raw=await client.request('GET',`/sessions/${s.sessionId}/transcript`); if(raw&&raw.text)$('asr-raw-text').textContent=raw.text; $('asr-raw-view').open=true; if(raw&&raw.path&&fs.existsSync(raw.path))file=raw.path; }catch(_){/* fall back to disk next to document */} } if(!file)file=resolveTranscriptFile(); $('asr-raw-view').open=true; const abs=path.resolve(file); const electron=w.reqnode('electron'); let err=''; if(electron&&electron.shell&&electron.shell.openPath){ err=await electron.shell.openPath(abs); }else{ err='no shell.openPath'; } if(err){ w.reqnode('child_process').execFileSync('cmd.exe',['/c','start','',abs],{windowsHide:true,stdio:'ignore'}); } hooks.status('\u5df2\u6253\u5f00\uff1a'+path.basename(abs)); }catch(e){hooks.status(e.message||String(e));} } more.addEventListener('click',function(ev){ const t=ev.target; if(!t||!t.closest)return; if(t.closest('#asr-raw-open')){ev.preventDefault();openTranscriptFile();} }); 
 ...
 check();loadConfig().catch(()=>{});loadAsrConfig().catch(()=>{});const timer=w.setInterval(()=>{check();render(hooks.session(),hooks.pending(),hooks.toSave());},1500);
