...
module.exports=function controls(w,config,client,panel,hooks){
 ...
 $('asr-raw-open').onclick=async()=>{ const s=hooks.session(); if(!s||!s.sessionId){hooks.status('\u8bf7\u5148\u5f00\u59cb\u6216\u6062\u590d\u4e00\u4e2a\u5f55\u97f3\u4f1a\u8bdd');return;} $('asr-raw-open').disabled=true; try{ hooks.status('\u6b63\u5728\u6253\u5f00\u9010\u5b57\u7a3f\u2026'); const raw=await client.request('GET',`/sessions/${s.sessionId}/transcript`); $('asr-raw-text').textContent=raw.text||''; $('asr-raw-view').open=true; let file=raw.path; if(!file&&s.path){ const dir=path.dirname(s.path); const base=path.basename(s.path,path.extname(s.path)); const hit=(fs.readdirSync(dir)||[]).filter(n=>n.includes('\u9010\u5b57\u7a3f')&&n.startsWith(base)&&n.endsWith('.md')).sort().pop(); if(hit)file=path.join(dir,hit); } if(!file){hooks.status('\u9010\u5b57\u7a3f\u8def\u5f84\u4e0d\u53ef\u7528\uff0c\u8bf7\u5148\u91cd\u542f\u8f6c\u5199\u670d\u52a1');return;} if(!fs.existsSync(file)){hooks.status('\u9010\u5b57\u7a3f\u6587\u4ef6\u5c1a\u4e0d\u5b58\u5728');return;} const library=w.File&&w.File.editor&&w.File.editor.library; if(library&&typeof library.openFileInNewWindow==='function'){ library.openFileInNewWindow(file,false); }else if(library&&typeof library.openFile==='function'){ library.openFile(file); }else{ const electron=w.reqnode('electron'); const err=await electron.shell.openPath(file); if(err)throw new Error(err); } hooks.status('\u5df2\u6253\u5f00\u9010\u5b57\u7a3f\uff1a'+path.basename(file)); }catch(e){hooks.status(e.message||String(e));} finally{$('asr-raw-open').disabled=false;} };
 const normalize=s=>(s||'').replace(/^\uFEFF/,'').replace(/\r\n/g,'\n');
 function render(s,pending,toSave){
  $('asr-raw').disabled=$('asr-retry').disabled=!s;$('asr-raw-open').disabled=false;
