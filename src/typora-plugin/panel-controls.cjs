...
module.exports=function controls(w,config,client,panel,hooks){
 ...
 $('asr-raw-open').onclick=async()=>{ const s=hooks.session(); hooks.status('\u6b63\u5728\u6253\u5f00\u9010\u5b57\u7a3f\u2026'); try{ if(!s||!s.sessionId)throw new Error('\u8bf7\u5148\u5f00\u59cb\u6216\u6062\u590d\u4e00\u4e2a\u5f55\u97f3\u4f1a\u8bdd'); const raw=await client.request('GET',`/sessions/${s.sessionId}/transcript`); $('asr-raw-text').textContent=raw.text||''; $('asr-raw-view').open=true; let file=raw&&raw.path; if((!file||!fs.existsSync(file))&&s.path){ const dir=path.dirname(s.path); const base=path.basename(s.path,path.extname(s.path)); const marker='\u9010\u5b57\u7a3f'; const names=fs.readdirSync(dir).filter(n=>n.endsWith('.md')&&n.indexOf(marker)>=0&&n.indexOf(base)===0); names.sort(); if(names.length)file=path.join(dir,names[names.length-1]); } if(!file)throw new Error('\u627e\u4e0d\u5230\u9010\u5b57\u7a3f\u6587\u4ef6\uff0c\u8bf7\u5148\u91cd\u542f\u8f6c\u5199\u670d\u52a1\u540e\u518d\u8bd5'); if(!fs.existsSync(file))throw new Error('\u9010\u5b57\u7a3f\u6587\u4ef6\u5c1a\u4e0d\u5b58\u5728\uff1a'+file); const abs=path.resolve(file); let err=''; try{err=await w.reqnode('electron').shell.openPath(abs);}catch(e){err=e.message||String(e);} if(err){ try{w.reqnode('child_process').execFileSync('cmd.exe',['/c','start','',abs],{windowsHide:true,stdio:'ignore'});err='';} catch(e2){err=(err||'')+' / '+(e2.message||String(e2));} } if(err)throw new Error('\u65e0\u6cd5\u6253\u5f00\uff1a'+err); hooks.status('\u5df2\u6253\u5f00\u9010\u5b57\u7a3f\uff1a'+path.basename(abs)); }catch(e){hooks.status(e.message||String(e));} };
