...
module.exports=function controls(w,config,client,panel,hooks){
 ...
 $('asr-raw-open').onclick=async()=>{ const s=hooks.session(); if(!s){hooks.status('请先开始或恢复一个录音会话');return;} $('asr-raw-open').disabled=true; try{ hooks.status('正在打开逐字稿…'); const raw=await client.request('GET',`/sessions/${s.sessionId}/transcript`); $('asr-raw-text').textContent=raw.text||''; $('asr-raw-view').open=true; const file=raw.path; if(!file){hooks.status('逐字稿路径不可用');return;} if(!fs.existsSync(file)){hooks.status('逐字稿文件尚不存在：'+file);return;} let opened=false; try{ const library=w.File?.editor?.library; if(library?.openFile){library.openFile(file);opened=true;} else if(library?.openFileInNewWindow){library.openFileInNewWindow(file,false);opened=true;} }catch(e){/* fall through */} if(!opened){ try{ const electron=w.reqnode('electron'); const err=await electron.shell.openPath(file); if(err)throw new Error(err); opened=true; }catch(e){ await new Promise((resolve,reject)=>{ const child=spawn('explorer.exe',[file],{windowsHide:true,shell:false,stdio:'ignore'}); child.once('error',reject); child.once('spawn',resolve); setTimeout(resolve,500); }); opened=true; } } hooks.status(opened?'已打开逐字稿：'+path.basename(file):'无法打开逐字稿'); }catch(e){hooks.status(e.message||String(e));} finally{$('asr-raw-open').disabled=false;} };
