...
module.exports=function controls(w,config,client,panel,hooks){
 ...
 $('asr-storage-pick').onclick=async()=>{
  ...
  try{
   ...
   const chosen=await pickFolder(w,{title:'选择资料记录目录',defaultPath:current&&fs.existsSync(current)?current:''});
 ...
 $('asr-config-export').onclick=async()=>{
  ...
  try{
   ...
   let file=await pickSaveJson(w,{title:'导出配置',defaultPath:path.join(w.reqnode('os').homedir(),'Documents',name)});
 ...
 $('asr-config-import').onclick=async()=>{
  ...
  try{
   ...
   const file=await pickOpenJson(w,{title:'导入配置',defaultPath:path.join(w.reqnode('os').homedir(),'Documents')+path.sep});
