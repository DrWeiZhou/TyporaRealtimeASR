...
public sealed partial class Ledger {
 ...
 public object PolishStatus(string session){lock(gate){using var c=db.CreateCommand();c.CommandText="SELECT COUNT(*),COALESCE(SUM(CASE WHEN COALESCE(p.attempts,0)>=3 THEN 1 ELSE 0 END),0),COALESCE(MAX(p.error),'') FROM events e LEFT JOIN polish p ON p.id=e.id WHERE e.session=$0 AND e.state IN ('recognized','reviewed') AND p.result IS NULL";c.Parameters.AddWithValue("$0",session);using var r=c.ExecuteReader();r.Read();using var completed=db.CreateCommand();completed.CommandText="SELECT COUNT(*) FROM events e JOIN polish p ON p.id=e.id WHERE e.session=$0 AND p.result IS NOT NULL";completed.Parameters.AddWithValue("$0",session);return new {pending=r.GetInt64(0),failed=r.GetInt64(1),error=r.GetString(2),completed=(long)completed.ExecuteScalar()!};}}
 public List<TranscriptEvent> ReadyEvents(string session,long after){lock(gate){
  var result=new List<TranscriptEvent>();
  foreach(var e in Events(session,after)){
   if(e.State=="no_text"){result.Add(e with{State="deleted"});continue;}
   using var c=db.CreateCommand();c.CommandText="SELECT result,attempts FROM polish WHERE id=$0";c.Parameters.AddWithValue("$0",e.EventId);
   using var r=c.ExecuteReader();var exists=r.Read();var polished=exists&&!r.IsDBNull(0)?r.GetString(0):null;var failed=exists&&r.GetInt32(1)>0;
   if(polished==null && e.State is "recognized" or "reviewed"){
    if(failed){result.Add(e with{Text="",PolishState="failed"});continue;}
    break;
   }
   result.Add(e with{Text=polished??e.Text});
  }
  return result;
 }}
