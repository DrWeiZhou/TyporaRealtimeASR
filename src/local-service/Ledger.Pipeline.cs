...
public sealed partial class Ledger {
 ...
 public List<TranscriptEvent> ReadyEvents(string session,long after){lock(gate){var result=new List<TranscriptEvent>();foreach(var e in Events(session,after)){if(e.State=="no_text"){result.Add(e with{State="deleted"});continue;}using var c=db.CreateCommand();c.CommandText="SELECT result FROM polish WHERE id=$0";c.Parameters.AddWithValue("$0",e.EventId);var polished=c.ExecuteScalar() as string;if(polished==null && e.State is "recognized" or "reviewed")break;result.Add(e with{Text=polished??e.Text});}return result;}}
