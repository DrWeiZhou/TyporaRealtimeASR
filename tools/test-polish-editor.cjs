...
module.exports=async function(w){
 ...
 for(let i=0;i<60;i++){await wait(250);if(w.File.editor.getMarkdown().includes('已润色的接续内容。')&&w.typoraRealtimeAsr.getState().toSave===0&&fs.readFileSync(target,'utf8')===w.File.editor.getMarkdown())break;}
 ...
 const report={manualPreserved:first.includes('人工未保存改写必须保留'),polishedInserted:first.includes('已润色的第二句话。'),rawBlocked:!first.includes('原始口语')&&!first.includes('不能入文'),numberedIdeas:/^\d+\. 已润色的第一句话。$/m.test(first)&&/^\d+\. 已润色的第二句话。已润色的接续内容。$/m.test(first)&&!first.includes('00:00:00–00:00:01'),saved:disk===first,replayNoDuplicate:first===after,rawTimestamp:raw.includes('10:00:00')&&raw.includes('原始口语一'),controlsPresent:['asr-pause','asr-level','asr-service-start','asr-api-key','asr-file-save','asr-storage-dir','asr-config-export','asr-config-import'].every(id=>!!w.document.getElementById(id)),saveState:w.document.querySelector('#asr-file-save').textContent};
